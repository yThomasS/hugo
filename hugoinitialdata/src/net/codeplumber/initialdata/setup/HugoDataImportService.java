...
/**
 * @author thomas.schroeder@hybris.com on 2018-03-06.
 */
public abstract class HugoDataImportService {
     ...
     * Execute import data.
     *
     * @param systemSetup the system setup parameters
     * @param context     the system setup context
     ...
     * @param syncCatalogs executes catalog synchronization job if true
    ...
    {
        systemSetup.logInfo(context, String.format("Begin (hugo) synchronizing Product Catalog [%s]", catalogName));

        getSetupSyncJobService().createProductCatalogSyncJob(String.format("%s", catalogName));
    ...
    public boolean synchronizeContentCatalog(final AbstractSystemSetup systemSetup, final SystemSetupContext context,
                                             final String catalogName, final boolean syncCatalogs)
    {
        systemSetup.logInfo(context, String.format("Begin (hugo) synchronizing Content Catalog [%s]", catalogName));
     ...
     *
     * @param dependsOnProduct  the job to assign to
     * @param dependentContents the dependent job names
     */
    public void assignDependent(final String dependsOnProduct, final List<String> dependentContents) {
        if (CollectionUtils.isNotEmpty(dependentContents) && StringUtils.isNotBlank(dependsOnProduct)) {
            ...
            for (final String content : dependentContents) {
                dependentSyncJobsNames.add(String.format("%s", content));
            }

            getSetupSyncJobService().assignDependentSyncJobs(String.format("%s", dependsOnProduct),
     ...
     *
     * @param syncCronJobResult the sync job result
     ...
     *
     * @param extensionNameToCheck the extension name
