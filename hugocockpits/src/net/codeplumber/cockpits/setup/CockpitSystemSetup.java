 ...
 */
package net.codeplumber.cockpits.setup;
...
import net.codeplumber.cockpits.constants.HugoCockpitsConstants;
 ...
 */
@SystemSetup(extension = HugoCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(HugoCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(HugoCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						HugoCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/hugocockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
